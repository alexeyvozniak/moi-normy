(()=>{
  ...
  function normalizePrayerItems(){
    let changed=false;
    for(const item of state.items||[]){if(!isPrayer(item))continue;if(Number(item.quick)!==100){item.quick=100;changed=true;}}
    if(changed)save();
  }
  ...
  function configureEditor(){
    const type=$('practiceType')?.value||'',quick=$('eQuick'),inc=$('eIncrement'),quickField=quick?.closest('.field'),prayer=type==='prayer';
    if(quick){if(prayer)quick.value='100';quick.readOnly=prayer;quick.setAttribute('aria-readonly',prayer?'true':'false');}
    quickField?.classList.toggle('prayerFixedField',prayer);
    if(inc){if(prayer){inc.min='100';inc.step='100';}else{inc.removeAttribute('min');inc.removeAttribute('step');}}
  }

  function validatePrayerEditor(event){
    if(($('practiceType')?.value||'')!=='prayer')return;
    const increment=Number($('eIncrement')?.value);
    if(!validHundreds(increment)){event.preventDefault();event.stopImmediatePropagation();notice('Норма задаётся сотнями','Для молитвенной практики укажи 100, 200, 300 и т. д.');return;}
    if($('eQuick'))$('eQuick').value='100';
  }

  function openPrayerAmount(id){
    const item=itemById(id);if(!isPrayer(item))return;const available=fullHundreds(item.debt);
    ...
    amountPrayerId=id;openAmount(id);
  ...
  function handleCardAction(event,button){
    const item=itemById(button.dataset.id);if(!isPrayer(item))return false;const action=button.dataset.action;if(!['amount','quick','close'].includes(action))return false;
    event.preventDefault();event.stopPropagation();
    if(action==='amount')openPrayerAmount(item.id);
    else if(action==='quick'){if(Number(item.debt)<100)notice('Нет полной сотни',`Осталось ${Math.max(0,Number(item.debt)||0)} ${item.unit||'молитв'}.`);else subtract(item.id,100,'quick');}
    else{const amount=fullHundreds(item.debt);if(amount>=100)subtract(item.id,amount,'close');else notice('Нет полной сотни','Неполный остаток нельзя отметить выполненным из карточки.');}
    return true;
  }

  function handleAmountSave(event){
    if(!amountPrayerId)return false;const item=itemById(amountPrayerId);if(!isPrayer(item)){amountPrayerId=null;return false;}
    event.preventDefault();event.stopImmediatePropagation();const amount=Number($('amountInput')?.value),available=fullHundreds(item.debt);
    if(!validHundreds(amount)||amount>available){notice('Только полные сотни',`Укажи 100, 200, 300… не больше ${available}.`);return true;}
  ...
  function init(){
    ...
    window.addEventListener('pravilo:editor-saved',event=>{const item=itemById(event.detail?.id);if(isPrayer(item)&&Number(item.quick)!==100){item.quick=100;save();render();}});
  ...
  window.PraviloPrayerCard=Object.freeze({isPrayer,fullHundreds,validHundreds});
