(()=>{
  ...
  function ensureScreen(){
    ...
    root.innerHTML=`<div class="practiceBackdrop" id="meditationBackdrop"></div><div class="practiceVeil"></div><div class="practiceTop"><div class="practiceTopLabel">Медитация</div><button class="practiceExit" id="meditationExit" type="button">Вернуться</button></div>
      ...
      <div class="meditationDone"><div class="meditationDoneMark">済</div><div class="meditationDoneTitle">Медитация завершена</div><div class="meditationDoneText" id="meditationDoneText"></div><div class="hidden" id="meditationNoteFields" style="width:min(430px,100%);text-align:left;margin-top:18px"><div class="featureField"><label>Название / тема медитации</label><input id="meditationFinalTopic" placeholder="Можно назвать её теперь, если тема прояснилась в конце"></div><div class="featureField"><label>Заметка — необязательно</label><textarea id="meditationNoteBody" placeholder="Если хочется что-то сохранить…"></textarea></div></div><div class="meditationControls"><button class="meditationControl" id="meditationAddNote" type="button">Добавить заметку</button><button class="meditationControl primary" id="meditationDoneClose" type="button">Готово</button></div></div>`;
    document.body.appendChild(root);
    $('meditationExit').addEventListener('click',()=>close(false));
    $('meditationStart').addEventListener('click',start);
    $('meditationPause').addEventListener('click',pause);
    $('meditationFinish').addEventListener('click',()=>{if(confirm('Закончить медитацию сейчас?'))close(false);});
    $('meditationAddNote').addEventListener('click',()=>{const fields=$('meditationNoteFields');fields.classList.remove('hidden');if(!$('meditationFinalTopic').value)$('meditationFinalTopic').value=session?.topic||'';$('meditationFinalTopic').focus();});
    $('meditationDoneClose').addEventListener('click',saveNoteAndClose);
  }
  ...
  function update(){
    ...
    $('meditationPhase').textContent=session.phase===1?'первая часть · тишина':'вторая часть · тема';
    $('meditationPhaseTitle').textContent=session.phase===1?'Тишина':(session.topic||'Размышление');
    $('meditationPhaseTopic').textContent=session.phase===1?'Пятнадцать минут без задачи и без счёта.':(session.topic?`Тема: ${session.topic}`:'Останься с темой, которая проясняется сейчас.');
  }
  ...
  function complete(){
    ...
    $('meditationDoneText').textContent=`15 минут тишины и 15 минут размышления${session.topic?` над темой «${session.topic}»`:''}.`;
    $('meditationFinalTopic').value=session.topic||'';$('meditationPractice').classList.remove('running');$('meditationPractice').classList.add('completed');
  }
  function saveNoteAndClose(){
    if(!session)return;const title=$('meditationFinalTopic').value.trim()||session.topic||'',body=$('meditationNoteBody').value.trim();
