(()=>{
  ...
  function inferredType(item){
    ...
    if(/иисус|молитв|четк|чётк/i.test(`${item.name||''} ${item.unit||''}`))return 'prayer';
    if(/медит|созерц|тишин/i.test(`${item.name||''} ${item.unit||''}`))return 'meditation';
    return '';
  }
  ...
  function fill(id){ensureField();editingId=id||null;const item=id?state.items.find(x=>x.id===id):null;if($('practiceType'))$('practiceType').value=inferredType(item);}
  function prepare(){pendingType=$('practiceType')?.value||'';}
  function finish(event){const item=state.items.find(x=>x.id===event.detail?.id);if(!item)return;if(pendingType)item.practiceType=pendingType;else delete item.practiceType;save();render();pendingType='';}
  function migrate(){let changed=false;for(const item of state.items||[]){if(item.practiceType)continue;const type=inferredType(item);if(type){item.practiceType=type;changed=true;}}if(changed)save();}
