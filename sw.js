...
self.addEventListener('fetch',e=>{
  if(e.request.mode==='navigate'){
    e.respondWith(
      fetch(e.request)
        .then(r=>r.text())
        .then(text=>new Response(polishedHtml(text),{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache'}}))
        .catch(()=>caches.match('./index.html').then(r=>r.text()).then(text=>new Response(polishedHtml(text),{headers:{'Content-Type':'text/html; charset=utf-8'}})))
    );
    return;
  }
