...
self.addEventListener('notificationclick',event=>{event.notification.close();event.waitUntil((async()=>{const list=await clients.matchAll({type:'window',includeUncontrolled:true});const open=list.find(client=>'focus' in client);if(open)return open.focus();if(clients.openWindow)return clients.openWindow('./');})());});

function canonicalRequest(request){const url=new URL(request.url);if(url.origin!==self.location.origin)return null;url.search='';url.hash='';return new Request(url.toString(),{method:'GET',credentials:'same-origin'});}
async function matchCached(request,fallback){const exact=await caches.match(request);if(exact)return exact;const withoutQuery=await caches.match(request,{ignoreSearch:true});if(withoutQuery)return withoutQuery;if(fallback){const fallbackExact=await caches.match(fallback);if(fallbackExact)return fallbackExact;return caches.match(fallback,{ignoreSearch:true});}return null;}
async function putIfUsable(request,response){if(!response?.ok)return response;const url=new URL(request.url);if(url.origin!==self.location.origin)return response;const cache=await caches.open(CACHE);cache.put(request,response.clone()).catch(()=>{});const canonical=canonicalRequest(request);if(canonical&&url.search)cache.put(canonical,response.clone()).catch(()=>{});return response;}
async function networkFirst(request,fallback){try{const response=await fetch(request,{cache:'no-store'});if(!response?.ok)throw new Error(`HTTP ${response?.status||0}`);return await putIfUsable(request,response);}catch(error){return (await matchCached(request,fallback))||Response.error();}}
async function staleWhileRevalidate(request){const cached=await matchCached(request);const refresh=fetch(request).then(response=>putIfUsable(request,response)).catch(()=>null);return cached||(await refresh)||Response.error();}
...
self.addEventListener('fetch',event=>{
  ...
  const codeAsset=sameOrigin&&(request.destination==='script'||request.destination==='style'||/\.(?:js|css|webmanifest)$/i.test(url.pathname));
