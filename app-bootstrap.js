(()=>{
  ...
  /* Порядок важен: сначала UI-функции, затем режимы, затем инфраструктура. */
  const scripts=[
    'onboarding.js?v=4',
    'features.js?v=1',
    'reading-mode-fix.js?v=3',
    'history-delete.js?v=1',
    'enhancements.js?v=1',
    'practice.js?v=1',
    'practice-types.js?v=1',
  ...
  function ensureStyle(href){
    ...
    link.rel='stylesheet';link.href=href;link.dataset.praviloStyle=plain;
  ...
  function loadScript(src){
    return new Promise(resolve=>{
      ...
      if(document.querySelector(`script[data-pravilo-module="${plain}"]`)||[...document.scripts].some(s=>s.src&&s.src.includes(plain))){
        diagnostics.loaded.push(plain);resolve();return;
      }
      const s=document.createElement('script');
      s.src=src;s.async=false;s.dataset.praviloModule=plain;
      s.onload=()=>{diagnostics.loaded.push(plain);resolve();};
      s.onerror=()=>{diagnostics.failed.push(plain);console.error('[Правило] Не загрузился модуль:',plain);resolve();};
