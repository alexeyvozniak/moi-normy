(()=>{
  ...
  function openNote(id){
    ensureOverlay();const entry=state.history.find(h=>String(h.id)===String(id));if(!entry)return;entryId=entry.id;const kind=kindOf(entry),item=itemById(entry.itemId);
  ...
  function decorateHistory(){
    ...
    rows.forEach((row,i)=>{row.querySelectorAll('.notesExtra').forEach(x=>x.remove());const entry=entries[i];if(!entry)return;const kind=kindOf(entry);if(!kind)return;
      if(entry.note){
        const n=document.createElement('div');n.className='historyNote notesExtra';
        n.innerHTML=`<div class="historyNoteTag">${kind==='meditation'?'медитация':'чтение'}</div>${entry.note.title?`<div class="historyNoteTitle">${escapeHtml(entry.note.title)}</div>`:''}${entry.note.body?`<div class="historyNoteBody">${escapeHtml(entry.note.body)}</div>`:''}<div class="historyNoteActions"><button type="button" class="noteEditButton" data-edit-note-id="${escapeHtml(entry.id)}">✎ Изменить</button><button type="button" class="noteShareButton" data-share-history-id="${escapeHtml(entry.id)}">Поделиться</button></div>`;
        row.appendChild(n);
  ...
  function watchActions(){
    document.addEventListener('click',e=>{
      const edit=e.target.closest?.('[data-edit-note-id]');if(edit){e.preventDefault();e.stopPropagation();openNote(edit.dataset.editNoteId);return;}
      const b=e.target.closest?.('button');if(!b)return;const taskAction=b.closest?.('.task [data-action]')||b.id==='amountSave';if(!taskAction)return;const before=state.history[0]?.id||null;setTimeout(()=>maybePromptAfterAction(before),40);
    },true);
  }
  function init(){ensureOverlay();decorateHistory();watchActions();window.addEventListener('pravilo:render',decorateHistory);}
  window.PraviloNotes=Object.freeze({openNote});
